...
def transform_annotations(data):
    result = []
    for task in data:
        file_name = task.get("file_upload", None) 
        paragraphs = []
    
        # annotations = {item["id"]: item for item in task["annotations"][0]["result"] if "id" in item}    
        ...
        for item in task["annotations"][0]["result"]:
            if item.get("from_name") == "label" and "Paragraph" in item["value"]["labels"]:
                ...
                for trans in task["annotations"][0]["result"]:
                ...
                for word_item in task["annotations"][0]["result"]:
                    if word_item.get("parentID") == item["id"] and word_item.get("from_name") == "label" and "Word" in word_item["value"]["labels"]:
                        ...
                        for char_item in task["annotations"][0]["result"]:
